...
export function NewPeerButton({ tenantSlug }: { tenantSlug: string }) {
 ...
 className="rounded-md bg-bamboo-50 px-3 py-1.5 text-sm font-medium text-ink-950 transition-colors hover:bg-ink-800"
 >
 {t('button')}
 </button>
 {open && (
...
function FormView({
 ...
 <span className="text-bamboo-100">{t('descriptionLabel')}</span>
 <input
 type="text"
 value={description}
 disabled={pending}
...
function ResultView({
 ...
 <div className="flex flex-col items-center gap-2 rounded-md border border-ink-800 bg-ink-950 px-4 py-4 dark:bg-ink-900">
 ...
 <p className="text-center text-xs text-bamboo-200/60">{t('qrHint')}</p>
 </div>

 <details className="space-y-2 text-sm">
 <summary className="cursor-pointer select-none text-xs uppercase tracking-wide text-bamboo-200/60 hover:text-bamboo-100 dark:text-bamboo-200/40 dark:hover:text-bamboo-100">
...
function CopyableField({
 ...
 type="button"
 onClick={copy}
 className="rounded border border-bamboo-200/30 px-2 text-xs text-bamboo-100 hover:bg-zinc-50 dark:text-bamboo-100"
 >
