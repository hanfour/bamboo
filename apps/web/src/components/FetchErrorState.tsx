...
export function FetchErrorState({
 ...
 {message ? (
 <p className="mt-2 font-mono text-xs text-bamboo-200/40">{message}</p>
 ) : null}
 </ErrorCard>
 );
 }
...
function ErrorCard({
 ...
 return (
 <div
 className={`rounded-md border border-ink-800 bg-ink-900/40 p-6 ${className}`}
 >
 ...
 <p className="mt-1 text-sm text-bamboo-200/70">{body}</p>
...
const OUTLINED_BUTTON =
 'inline-flex items-center rounded-md border border-bamboo-200/30 px-3 py-1.5 text-sm font-light text-bamboo-100 transition-colors hover:border-bamboo-200/60 hover:text-bamboo-50';
