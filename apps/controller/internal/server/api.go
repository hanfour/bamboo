...
func (h *HTTPServer) routeAPI(w http.ResponseWriter, r *http.Request) {
	...
	// Dispatch to the per-resource routers (api_router.go). Each returns
	// true once it owns — and has answered — the path; the order matches
	// the original if-chain. routeReadOnlyAndDNS is terminal (handles the
	// GET-only reads, /dns, and the canonical no-route 404).
	switch {
	case h.routePeersSub(w, r, authn, tenant):
	case h.routePreAuthKeys(w, r, authn, tenant):
	case h.routeInvitations(w, r, authn, tenant):
	case h.routeAPITokens(w, r, authn, tenant):
	case h.routeWebhooks(w, r, authn, tenant):
	case h.routePolicy(w, r, authn, tenant):
	default:
		h.routeReadOnlyAndDNS(w, r, authn, tenant)
	}
}
