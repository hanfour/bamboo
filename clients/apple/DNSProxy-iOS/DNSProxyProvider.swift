...
// iOS variant of the MagicDNS proxy. The implementation mirrors
// clients/apple/DNSProxy-macOS/DNSProxyProvider.swift because
// NEDNSProxyProvider has the same surface on both platforms.
// Maintaining two files rather than a single Shared/ source is a
// deliberate choice: xcodegen places extension targets into
// separate Swift modules, and the principal class lookup is per-
// module. Easier to duplicate than to plumb cross-module visibility.
...
private final class DNSFlowHandler {
    ...
    /// finishWriteClose tears down the write half. Invoked exactly once —
    /// by whichever of read-EOF or the last draining response wins the
    /// FlowLifecycle latch — so a pending synthesised write is never cut
    /// off mid-flight. (No handler Set to release on iOS; the capturing
    /// closures keep the handler alive until the last one completes.)
